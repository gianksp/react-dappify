...
class WalletLinkConnector extends Moralis?.AbstractWeb3Connector {
  ...
  walletLink = new WalletLink({
    appName:
      getPreference(constants.PREFERENCES.SUBDOMAIN)?.config?.name ||
      defaultConfiguration.name,
    appLogoUrl:
      getPreference(constants.PREFERENCES.SUBDOMAIN)?.config?.icon ||
      defaultConfiguration.icon,
    darkMode: false
  })
  ...
  async activate() {
    ...
    const appConfiguration = getPreference(
      constants.PREFERENCES.SUBDOMAIN
    )?.config
