...
export default class Nft {
  ...
  #fromProvider = (nft) => {
    Logger.debug('NFT from provider')
    Logger.debug(nft)
    this.id = nft.id
  ...
  sellTo = async (price, category, qty = 1) => {
    ...
    const contractAddress =
      currentProject.config.template[process.env.REACT_APP_TEMPLATE_NAME]
        .contract[chainId]
    ...
    if (this.type === 'ERC721') {
      ...
      if (!isApprovedForAll) {
        const txApprove = await tokenContract.setApprovalForAll(
          contractAddress,
          true
        )
        await txApprove.wait()
    ...
    const transaction = await marketplace.contract.placeOffering(
      this.collection.address,
      this.tokenId,
    ...
    const event = tx.events.filter((e) => e.event === STATUS.OFFERING_PLACED)[0]
      .args
  ...
  purchase = async (qty = 1) => {
    ...
    const transaction = await marketplace.contract.closeOffering(
      this.offeringId,
      qty,
  ...
  editPricing = async (price) => {
    ...
    const transaction = await marketplace.contract.updateOffering(
      this.offeringId,
      newPricing
    )
    const tx = await transaction.wait()
  ...
  withdrawFromMarketplace = async () => {
    ...
    let marketplace = new Marketplace(contractAddress)
    marketplace = await marketplace.init()
    const transaction = await marketplace.contract.withdrawOffering(
      this.offeringId
    )
    const tx = await transaction.wait()
  ...
  refreshMetadata = async () => {
    ...
    const incompleteNft =
      !this?.source?.attributes?.chainId ||
  ...
  static getById = async (collectionAddress, tokenId) => {
    ...
    Logger.debug(nft)
    if (!isEmpty(nft)) {
      Logger.debug('Found nft in Dappify db')
      return new Nft(nft)
    } else {
      Logger.debug('Fetching nft data from api')
      const nftContent = await Nft.fetchFromAPI(collectionAddress, tokenId)
      return new Nft(nftContent)
    }
  }
  ...
  static fetchFromAPI = async (collectionAddress, tokenId) => {
    ...
    try {
      ...
      Logger.debug(`Data found fetching nft api`)
      Logger.debug(nft)
    } catch (e) {
      Logger.error(e)
    }
  ...
  static getFromUser = async (userProfile, addressListFilter = []) => {
    ...
    const tokenAddresses =
      addressListFilter && addressListFilter.length > 0
        ? addressListFilter.map((addr) => `&token_addresses=${addr}`).join('')
        : ''
    ...
    try {
      items = await axios.get(
        `https://deep-index.moralis.io/api/v2/${userProfile.wallet}/nft?chain=${chainId}&format=decimal${tokenAddresses}`,
        {
          headers: {
            'X-API-Key': process.env.REACT_APP_MORALIS_API_KEY,
            accept: 'application/json'
  ...
  static fullTextSearch = async (text) => {
    ...
    const itemsFound = [
      ...items.map((item) => new Nft(item)),
      ...profiles.map((item) => new UserProfile(item))
    ]
    return itemsFound
  }
}
