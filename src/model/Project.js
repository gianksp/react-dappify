...
export default class Project {
  ...
  static load = async () => {
    ...
    if (uriComponents.length === 4) {
      sanitizedUrl = sanitizedUrl.replace(
        `${process.env.REACT_APP_TEMPLATE_NAME}.`,
        ''
      )
    }
    ...
    const isDappifySubdomain =
      domainName.domainWithoutSuffix.toLocaleLowerCase() ===
      Project.PLATFORM_DOMAIN
    const searchKey = isDappifySubdomain ? 'subdomain' : 'domain'
    const searchValue = isDappifySubdomain
  ...
  static loadFromProvider = async (searchKey, searchValue) => {
    ...
    Logger.debug(
      `Loading project from provider with searchKey ${searchKey} and value ${searchValue}`
    )
  ...
  static getTokenPrice = async () => {
    ...
    let tokenPrice
    try {
      tokenPrice = await axios.get(
        `https://deep-index.moralis.io/api/v2/erc20/${constants.PRICE_REF_ETH_MAINNET[chainId]}/price?chain=${chainId}`,
        {
          headers: {
