...
const getProjectFromUrl = async (url) => {
  ...
  const isDappifySubdomain =
    url.includes('dappify.com') ||
    url.includes('dappify.us') ||
    url.includes('dappify.cc')
  ...
  if (!originalContact && newContact !== originalContact) {
    ...
    if (targetWebhookUrl) {
      Moralis.Cloud.httpRequest({
        ...
        body: {
          content: `Give a warm welcome to ${object.get(
            'nickname'
          )} at his contact provided ${object.get('contact')}`
        }
      }).then(
...
Moralis.Cloud.beforeSave('Project', async (request) => {
  ...
  if (!object.id) {
    const config = await Moralis.Config.get({ useMasterKey: true })
    const targetWebhookUrl = config.get('newProjectDiscordWebhook')
    if (targetWebhookUrl) {
      // Get Owner
      Moralis.Cloud.httpRequest({
        method: 'POST',
        url: targetWebhookUrl,
        body: {
          content: `A new project has been created with subdomain ${object.get(
            'subdomain'
          )}`
        }
      }).then(
      ...
      logger.info(`New project webhook not configured`)
    }
  }
})

Moralis.Cloud.beforeSave('UserProfile', async (request) => {
  const { object } = request
  if (!object.id) {
    const config = await Moralis.Config.get({ useMasterKey: true })
    const targetWebhookUrl = config.get('newProfileDiscordWebhook')
    if (targetWebhookUrl) {
      // Get Owner
      Moralis.Cloud.httpRequest({
        method: 'POST',
        url: targetWebhookUrl,
        body: {
          content: `A new end user profile has been created`
        }
      }).then(
        function (httpResponse) {
          logger.info(httpResponse.text)
...
Moralis.Cloud.define('getProfileByHandle', async (request) => {
  ...
  const isweb3 = web3.utils.isAddress(handle)
  const query = new Moralis.Query('User')
  if (isweb3) query.equalTo('ethAddress', handle.toLowerCase())
  else query.equalTo('username', handle)
