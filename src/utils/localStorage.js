...
export const getProviderPreference = () => {
  ...
  if (authParams && authParams.connector) {
    ...
    Logger.debug(
      `Getting provider preferences ${authParams.connector} for ${CoinbaseConnector.name}`
    )
    Logger.debug(authParams.connector === CoinbaseConnector.name)
    authParams.connector =
      authParams.connector === CoinbaseConnector.name
        ? CoinbaseConnector
        : authParams.connector
  } else {
...
export const setProviderPreference = (authParams = {}) => {
  ...
  if (
    authParams &&
    authParams.connector &&
    authParams.connector instanceof Function
  ) {
    // Change from class to string for ref
